<x-layout-content class="w-full flex justify-around items-center fixed z-20 sm:static sm:block bottom-0 sm:w-[20%] overflow-hidden h-20 sm:rounded-lg sm:h-auto">
    {{-- Beranda --}}
    <x-nav-link class="flex items-center gap-2" href="{{ route('home') }}" :active="request()->routeIs('home')">
            <svg class="w-10 h-10 lg:w-8 lg:h-8" viewBox="0 0 23 27" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <x-nav-link class="flex items-center gap-2">
            <svg class="w-10 h-10 lg:w-8 lg:h-8" viewBox="0 0 31 25" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    {{-- Profile --}}
    @auth
       <x-nav-link :href="route('profile.show', Auth::user())" :active="request()->is('profile/' . Auth::user()->username ) || request()->is('profile/' . Auth::user()->username . '/bookmarks') || request()->routeIs('profile.edit')" class="flex items-center gap-2" >
            <svg class="w-10 h-10 lg:w-8 lg:h-8" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <div>
        ...
        <div class="sm:mx-4 sm:my-3">
          <button @click="open = true" class="bg-blue-600 hover:bg-blue-700 duration-300 text-lg md:text-2xl font-bold flex justify-center px-4 py-1 sm:w-full rounded-lg">SPILL</button>
        </div>
        ...
        style="display: none;">
            ...
            {{-- Modal Content --}}
            <div class="bg-white dark:bg-gray-800 w-11/12 md:w-[40%] rounded-lg p-6 relative" @click.outside="open = false">

                {{-- Form Content --}}
                <h2 class="sm:text-xl font-bold mb-4">What Do You Want To Spill Today ?</h2>
                <form action="{{ route('post.create') }}" method="POST" enctype="multipart/form-data">
                    @csrf

                    <div class="mb-4">
                        <textarea name="content" id="content" rows="6" cols="10" required
                            class="mt-1 text-sm sm:text-base block w-full border-none bg-transparent focus:ring-0 focus:border-none" placeholder="What's on your mind ?"></textarea>
