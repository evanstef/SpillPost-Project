...
<div class="space-y-4">
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-2">
            ...
            <a href="{{ route('profile.show', $post->user) }}" class="text-xs sm:text-base hover:underline hover:text-blue-600 duration-300 ease-in-out">{{ $post->user->username }}</a>
        </div>
        <div>
            <p class="text-xs sm:text-base">{{ $post->created_at->diffForHumans() }}</p>
        </div>
    </div>

    {{-- Post Content --}}
    <div>
        <p class="text-[11px] sm:text-base">{{ $post->body }}</p>
    </div>

    {{-- Post Image --}}
    @if ($post->image)
        <div class="w-full h-40 sm:h-[400px]">
    ...
    <div class="flex justify-around items-center">
        ...
        <form action="{{ Auth::check() && $post->likedByUsers->contains(Auth::user()->id) ? route('post.unlike', $post) : route('post.like', $post) }}" method="POST">
            ...
            <div class="flex items-center gap-2">
                ...
                <p class="text-[10px] sm:text-base">{{ $post->likedByUsers->count() }}</p>
            </div>
        ...
        <a class="flex items-center gap-2" href="{{ route('post.show', $post) }}">
            ...
            <p class="text-[10px] sm:text-base">{{ $post->comments->count() }}</p>
        </a>
