...
<div class="space-y-4">
    ...
    @if ($bookmark->post->image)
        <div class="w-full h-40 sm:h-[400px]">
            <img class="w-full h-full object-cover rounded-lg" src="{{ asset('storage/' . $bookmark->post->image) }}" alt="">
        </div>
    @endif
    ...
    <div class="flex justify-around items-center">
        ...
        <form action="{{ Auth::check() && $bookmark->post->likedByUsers->contains(Auth::user()->id) ? route('post.unlike', $bookmark->post) : route('post.like', $bookmark->post) }}" method="POST">
            ...
            <div class="flex items-center gap-2">
                ...
                <p class="text-[10px] sm:text-base">{{ $bookmark->post->likedByUsers->count() }}</p>
            </div>
