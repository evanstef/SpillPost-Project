        ...
        <div class="p-3 sm:p-6 space-y-4">
           ...
           <form action="{{ route('post.comment', $post) }}" method="POST">
                ...
                <div class="flex items-center gap-2">
                    <x-text-input class="w-[80%] py-1 md:py-2  text-[11px] md:text-base" name="comment" type="text" placeholder="Add a comment..." autocomplete="off"></x-text-input>
                    @error('comment')
                        <p class="text-red-500">{{ $message }}</p>
                    @enderror
                    <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-[9px] sm:text-[10px] md:text-base duration-300 ease-in-out text-white px-2 sm:px-3 py-1 rounded sm:rounded-lg w-[20%]">Submit</button>
                </div>
           </form>
